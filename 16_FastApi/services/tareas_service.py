...
def listar_tareas(prioridad,conexion, usuario_id):
    if prioridad is not None:
        return obtener_por_prioridad(prioridad,conexion, usuario_id)
    else:
        return obtener_todas(conexion, usuario_id)

def obtener_tarea(id, conexion, usuario_id):
    tarea_buscada = obtener_por_id(id, conexion,usuario_id)
    if tarea_buscada is None:
        raise HTTPException(status_code=404, detail="Tarea no encontrada")
    return tarea_buscada

def crear_tarea(tarea,usuario_id, conexion):
    return crear_tarea_db(tarea,usuario_id, conexion)

def actualizar_tarea(id, datos, conexion,usuario_id):
    tarea_buscada =obtener_por_id(id, conexion,usuario_id)
    if tarea_buscada is None:
            raise HTTPException(status_code=404, detail="Tarea no encontrada")
    ...
    return actualizar_tarea_db(id, titulo, prioridad, conexion, usuario_id)

def eliminar_tarea(id, conexion, usuario_id):
    tarea_borrada = eliminar_tarea_db(id, conexion, usuario_id)
